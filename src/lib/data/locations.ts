...
const EXPENSE_FK = 'location_id'

export async function listLocations(): Promise<CatalogItem[]> {
  ...
  if (error) { console.error('listLocations failed:', error.message); return [] }
...
export async function renameLocation(
  ...
  if (error) {
    ...
    console.error('renameLocation failed:', error.message)
    return { ok: false, error: 'save_failed' }
  }
  return { ok: true }
}
...
export async function deleteLocation(id: string): Promise<{ ok: boolean }> {
  ...
  if (error) { console.error('deleteLocation failed:', error.message); return { ok: false } }
  return { ok: true }
}

export async function countExpensesUsingLocation(id: string): Promise<number> {
  const m = await getMembership()
  if (!m) return 0
  const supabase = await createClient()
  ...
  if (error) { console.error('countExpensesUsingLocation failed:', error.message); return 0 }
  return count ?? 0
}
